...
def _array_candidates(
    data: Any,
) -> list[tuple[tuple[str, ...], list[Any], dict[str, Any] | None]]:
    if isinstance(data, list):
        return [((), data, None)]

    candidates: list[
        tuple[tuple[str, ...], list[Any], dict[str, Any] | None]
    ] = []
    ...
    def visit(
        ...
        if path and re.search(
            r"metadata|debug|errors?|warnings?|logs?",
            path_text,
            re.IGNORECASE,
        ):
            return
...
def _collect_deep_field_paths(
    value: Any,
    prefix: tuple[str, ...],
    output: set[tuple[str, ...]],
    depth: int = 0,
    max_depth: int = 12,
) -> None:
    if depth > max_depth:
...
def _candidate_score(path: tuple[str, ...], rows: list[Any]) -> float:
    ...
    path_hint = (
        ...
        if path
        and re.search(
            r"items|rows|list|data|result|payload",
            path[-1],
            re.IGNORECASE,
        )
        else 0
    ...
    return max(
        ...
        min(
            1,
            min(0.22, len(rows) / 50)
            + object_ratio * 0.34
            + min(0.24, leaf_count / 30)
            + path_hint
            - depth_penalty,
        ),
    )
...
def _rows_from_data(
    data: Any,
) -> tuple[list[Any], str, str | None, int]:
    ...
    if candidates:
        ...
        if primary_array_path == "items":
            return (
                ...
                item_shape,
                primary_array_path,
                row_count if row_count is not None else len(rows),
            )
        if primary_array_path:
            return (
                rows,
                f"object{{{primary_array_path}:array<object>}}",
                primary_array_path,
                row_count if row_count is not None else len(rows),
            )
...
def build_sample_data_preview(
    ...
    if primary_array_path:
        preview_data = _set_path(
            {},
            tuple(primary_array_path.split(".")),
            sample_rows,
        )
    elif isinstance(data, list):
        preview_data = sample_rows
    else:
        preview_data = sample_rows[0] if sample_rows else None

    while (
        sample_rows
        and _byte_length(preview_data) > byte_limit
    ):
    ...
    return {
        ...
        "truncated": len(rows) > len(sample_rows)
        or byte_length > byte_limit,
        "byteLength": byte_length,
        "maskedFields": sorted(masked_fields)[:100],
        "data": preview_data,
    }


def _field_type(examples: list[Any]) -> str:
    first = next(
        (value for value in examples if value is not None),
        None,
    )
...
def _field_format(
    ...
    first = next(
        (value for value in examples if isinstance(value, str)),
        None,
    )
    if not first:
        return None
    if (
        ...
        or first.startswith("/images/")
    ):
        return "image-url"
    if (
        re.search(r"url|uri", key, re.IGNORECASE)
...
def _roles(
    ...
    if key == "id" or key.endswith("Id"):
        roles.append("id")
    if re.search(
        r"name|title|label|equipmentName",
        key,
        re.IGNORECASE,
    ):
        roles.extend(["title", "label"])
    if re.search(
        r"description|content|summary|message|detail",
        key,
        re.IGNORECASE,
    ):
        roles.extend(["content", "description"])
    if (
        field_format == "image-url"
        or re.search(
            r"image|photo|thumbnail",
            key,
            re.IGNORECASE,
        ...
        field_format == "uri"
        or re.search(r"url|uri", key, re.IGNORECASE)
    ):
        roles.append("uri")
    ...
    if re.search(
        ...
        roles.append("status")
    if re.search(
        r"category|type|group|kind",
        key,
        re.IGNORECASE,
    ):
        roles.append("category")
    if re.search(
        r"location|zone|site",
        key,
        re.IGNORECASE,
    ):
...
def _capabilities(
    ...
    return {
        "hasImages": any(
            "image" in field["roles"]
            for field in fields
        ),
        "hasBooleans": any(
            field["type"] == "boolean"
            or "booleanFlag" in field["roles"]
            for field in fields
        ),
        "hasStatus": any(
            "status" in field["roles"]
            for field in fields
        ),
        "hasTimeField": any(
            "time" in field["roles"]
        ...
        "hasNumericMetrics": any(
            "metric" in field["roles"]
            or field["type"] == "number"
            for field in fields
        ),
        "hasCategories": any(
            "category" in field["roles"]
            for field in fields
        ),
        "hasNestedObjects": any(
            field["type"] in ["object", "array"]
            for field in fields
        ),
        "hasActions": any(
            "action" in field["roles"]
            for field in fields
        ),
    }


def build_derived_schema(
    data: Any,
    source_id: str = "unknown",
    sample_data_preview: dict[str, Any] | None = None,
) -> dict[str, Any]:
    preview = sample_data_preview or build_sample_data_preview(
        data,
        source_id=source_id,
    )
    rows, _, primary_array_path, _ = _rows_from_data(
        preview.get("data")
    )
    object_rows = [
    ...
    for field_path in _deep_field_paths(object_rows):
        key = field_path[-1]
        path_text = ".".join(field_path)
        if SENSITIVE_KEY_RE.search(path_text):
            continue
        examples = [
            value
            for value in (
                _row_value_at_path(row, field_path)
                for row in object_rows[:5]
            )
            if value is not None
        ]
        field_type = _field_type(examples)
        ...
        path = (
            f"{primary_array_path}.{path_text}"
            if primary_array_path
            else path_text
        )
        fields.append(
